...
function runCode(language, inputPath, outputPath, solutionFile, callback) {
    ...
    if (!solutionFile.endsWith(lang.fileExtension)) {
        return callback(new Error(`Incorrect solution file extension. Expected ${lang.fileExtension}, got ${path.extname(solutionFile)}`), null);
    }
    ...
    if (lang.name === 'python' || lang.name === 'javascript') {
        ...
        (0, child_process_1.exec)(compileCommand, (compileErr) => {
            if (compileErr) {
...
function executeCommand(command, outputFile, callback) {
    (0, child_process_1.exec)(command, { timeout: 10000 }, (err, stdout, stderr) => {
        if (err || stderr) {
            const errorMessage = stderr || err.message || 'Unknown execution error';
